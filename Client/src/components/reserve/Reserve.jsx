...
const Reserve = ({ hotelId }) => {
  ...
  const alldates1 = getDatesInRange(dates[0].startDate, dates[0].endDate);

  const isAvailable = (roomNumber) => {
    console.warn(roomNumber, alldates1)
    const isFound = roomNumber.unavailableDates.some((date) =>
      alldates1.includes(new Date(date).getTime())
    );
      ...
      for (let item of data) {
        ...
        for (let roomNumber of item.roomNumber) {
          if (room === roomNumber._id) {
            ...
            infoItem["id"] = room
            infoItem['date'] = [];
            infoItem["date"].push(JSON.parse(localStorage.getItem('date'))[0].startDate);
            infoItem["date"].push(JSON.parse(localStorage.getItem('date'))[0].endDate);
            cart.push(infoItem)
  ...
  const handleClickBook = (e) => {
    ...
    localStorage.setItem('bill', JSON.stringify([data[value]]))
    for (let room of selectedRooms) {
      let infoItem = {}
      for (let roomNumber of data[value].roomNumber) {
        if (room === roomNumber._id) {
          infoItem["title"] = data[value].title
          infoItem["price"] = data[value].price
          infoItem["desc"] = data[value].desc
          infoItem["maxPeople"] = data[value].maxPeople
          infoItem["room"] = roomNumber.number
          infoItem["id"] = room
          infoItem['date'] = [];
          infoItem["date"].push(JSON.parse(localStorage.getItem('date'))[0].startDate);
